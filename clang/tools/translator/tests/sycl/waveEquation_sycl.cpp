...
// CFL条件
const float dt = 0.5f * std::fmin(dx, dy) / c; // 满足稳定性条件
...
void waveEq(float* cur,float* prev,float* next,sycl::accessor<int, 1, sycl::access::mode::read_write> info_cur_acc, sycl::accessor<int, 1, sycl::access::mode::read_write> info_prev_acc, sycl::accessor<int, 1, sycl::access::mode::read_write> info_next_acc) 
{
    next[0] = 2.F * cur[1*info_cur_acc[1]+1] - prev[0] + c * c * dt * dt * ((cur[2*info_cur_acc[1]+1] - 2.F * cur[1*info_cur_acc[1]+1] + cur[0*info_cur_acc[1]+1] / (dx * dx)) + (cur[1*info_cur_acc[1]+2] - 2.F * cur[1*info_cur_acc[1]+1] + cur[1*info_cur_acc[1]+0] / (dy * dy)));
}


// 生成函数调用
void waveEqShell_waveEq(const dacpp::Tensor<float, 2> & matCur, const dacpp::Tensor<float, 2> & matPrev, dacpp::Tensor<float, 2> & matNext) { 
    // 设备选择
