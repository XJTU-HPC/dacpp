...
void PDE(dacpp::Tensor<int> u_kin, dacpp::Tensor<int> u_kout, dacpp::Tensor<int> r) { 
    ...
    // 设备内存分配
    int *d_u_kin=malloc_device<int>(303,q);
    // 设备内存分配
    int *d_u_kout=malloc_device<int>(151,q);
    // 设备内存分配
    ...
    q.submit([&](handler &h) {
        h.parallel_for(sycl::nd_range<3>(global * local, local),[=](sycl::nd_item<3> item) {
            ...
            const auto idx1=item_id/1467421616%0;
            const auto S1=item_id%1467421616;
            // 嵌入计算
			
            pde(d_u_kin+(idx1*101),d_u_kout+(idx1*101),d_r+());
        });
    }).wait();
    ...
    // 归约
    int *reduction_u_kin = malloc_device<int>(1212,q); //存归约结果 归约结果存在长度为ARRAY_SIZE的数组中
    q.submit([&](handler &h) {
    	h.parallel_for(
        range<1>(0 * 1212),
    ...
    // 归约
    int *reduction_u_kout = malloc_device<int>(606,q); //存归约结果 归约结果存在长度为ARRAY_SIZE的数组中
    q.submit([&](handler &h) {
    	h.parallel_for(
        range<1>(0 * 606),
        reduction(span<int,606>(reduction_u_kout,606), 
        {{REDUCTION_RULE}},
        property::reduction::initialize_to_identity()),
        [=](id<1> i,auto &reducer) {
    ...
    // 归约
    int *reduction_r = malloc_device<int>(6,q); //存归约结果 归约结果存在长度为ARRAY_SIZE的数组中
    q.submit([&](handler &h) {
    	h.parallel_for(
        range<1>(0 * 6),
    ...
    // 归并结果返回
    q.memcpy(r_u_kout, d_u_kout, 606*sizeof(int)).wait();
    // 归并结果返回
